...
int main()
{
    ...
    bool playAgain = true;
    int player_state; // Declare player_state outside the loop
    ...
    while (playAgain)
    {
        ...
        while (!game_is_done(board))
        {
            ...
            if (position_occupied(board, std::make_pair(row, col))) // Added missing closing parenthesis
            {
        ...
        player_state = get_board_state(board, PLAYER_MARKER); // Remove int declaration here
